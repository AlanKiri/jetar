  ...
  return (
    <Magnetic springOptions={{ bounce: 0 }} intensity={0.3}>
      <a
        href={link}
        className={cn(
          'group relative inline-flex shrink-0 items-center gap-[1px] rounded-full bg-zinc-100 px-2.5 py-1 text-sm text-black transition-colors duration-200 hover:bg-zinc-950 hover:text-zinc-50 dark:bg-zinc-800 dark:text-zinc-100 dark:hover:bg-zinc-700',
          className,
        )}
        target={target}
      >
          ...
          <Tooltip>
            <TooltipTrigger>
              <MagneticSocialLink
                target="_blank"
                link={CV.link}
                iconClassName={CV.cv_outdated ? 'text-yellow-600' : undefined}
              >
                CV
              </MagneticSocialLink>
            </TooltipTrigger>
            <TooltipContent>
              Updated{' '}
              {moment(CV.updated).isSame(new Date(), 'day')
                ? `Today ${CV.updated.toLocaleDateString()}`
                : `on ${CV.updated.toLocaleDateString()}`}
              {CV.cv_outdated && (
                <span className="text-yellow-400 dark:text-yellow-600">
                  {' '}
                  (outdated)
                </span>
              )}
      ...
      <section>
        ...
        <div className="flex flex-col space-y-2">
          {WORK_EXPERIENCE.map((job) => {
            const start = moment(job.start, 'YYYY.MM')
            let end = null

            if (job.end === 'Present') end = moment()
            else end = moment(job.end, 'YYYY.MM')
                  ...
                  <div className="relative flex w-full flex-row justify-between">
                    ...
                    <div className="flex gap-2">
                      ...
                      <svg
                        ...
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="h-6 w-6 my-auto text-zinc-600 dark:text-zinc-400"
                      >
                        {job.page && (
                          <path
                            d="M3.64645 11.3536C3.45118 11.1583 3.45118 10.8417 3.64645 10.6465L10.2929 4L6 4C5.72386 4 5.5 3.77614 5.5 3.5C5.5 3.22386 5.72386 3 6 3L11.5 3C11.6326 3 11.7598 3.05268 11.8536 3.14645C11.9473 3.24022 12 3.36739 12 3.5L12 9.00001C12 9.27615 11.7761 9.50001 11.5 9.50001C11.2239 9.50001 11 9.27615 11 9.00001V4.70711L4.35355 11.3536C4.15829 11.5488 3.84171 11.5488 3.64645 11.3536Z"
